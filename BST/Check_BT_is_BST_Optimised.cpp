...
bool ischeckBST2Helper(BinaryTreeNode *root, int min = INT_MIN, int max = INT_MAX)
{
    if (root == NULL)
    {
        return true;
    ...
    { //where min and max represrent upper an dlower bounds on our element
        return false;
    }
    return ischeckBST2Helper(root->left, min, root->data - 1) && ischeckBST2Helper(root->right, root->data + 1, max);
}
int main()
{
    BinaryTreeNode *root = takeInput();
    printLevelWise(root);
    cout << "Answer is " << ischeckBST2Helper(root);
    return 0;
}